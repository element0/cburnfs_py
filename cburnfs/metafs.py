...
import logging
logging.basicConfig(filename='log/MetaFS.log', encoding='utf-8', level=logging.DEBUG)

REDIS_CONFIG_PATH="etc/config-metafs.sh"

class MetaFS(FS):

    def __init__(self,
                 userhome_config_path=None,
                 redis_config_path=None,
                 config=None):
        super().__init__()
        _config = dict()
        if userhome_config_path == None:
            _config['USERPUBLICID'] = 'unset-user-public-id'
            _config['USERHOMENAME'] = 'unset-user-home-name'
            _config['USERFSURL'] = 'unset-userfs-url'
        else:
            _config = {
                **dotenv_values(userhome_config_path)
            }
        if redis_config_path == None:
            _config['REDIS_CONTAINER_NAME'] = 'localhost'
            _config['REDIS_CONTAINER_PORT'] = 6379
        else:
            _config.update({
        ...
        fsurn = f'fs:{_config["USERPUBLICID"]}:{_config["USERHOMENAME"]}:{_config["USERFSURL"]}'

        logging.debug(f"MetaFS: dotenv import complete. fsurn: {fsurn}")
        r = redis.Redis(
                _config['REDIS_CONTAINER_NAME'],
                _config['REDIS_CONTAINER_PORT'],
                db=0)
        self.redis = r

        logging.debug(f"Redis connection _configured. self.redis: {r}")
        ...
        if(fsnokey == None):
            ...
            r.set(fsnokey,
                json.dumps({
                    "root":inokey,
                    "urn":fsurn,
                    "user":_config['USERPUBLICID'],
                    "url":_config['USERFSURL']
                })
            )
